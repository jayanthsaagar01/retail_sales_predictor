...
def generate_sales_data(start_date='2023-01-01', end_date='2023-12-31', save_path=None):
    """
    Generate sales data with built-in patterns for high model accuracy.
    ...
    if save_path:
        ...
        print(f"Sales data saved to {save_path}")
    
    return df

def generate_weather_data(sales_data, location="Mumbai", save_path=None):
    """
    Generate weather data that correlates with sales patterns
...
def generate_sentiment_data(sales_data, keywords=["brand", "product", "quality"], save_path=None):
    """
    Generate sentiment data that correlates with sales patterns
...
def generate_all_data(output_dir='utils/data'):
    """
    Generate all datasets and save them to files
    ...
    sales_df = generate_sales_data(save_path=sales_path)
    
    # Generate weather data based on sales data
    weather_path = os.path.join(output_dir, 'weather_data.csv')
    weather_df = generate_weather_data(sales_df, save_path=weather_path)
    
    # Generate sentiment data based on sales data
    sentiment_path = os.path.join(output_dir, 'sentiment_data.csv')
    sentiment_df = generate_sentiment_data(sales_df, save_path=sentiment_path)
    ...
    with open(os.path.join(output_dir, 'data_metadata.json'), 'w') as f:
